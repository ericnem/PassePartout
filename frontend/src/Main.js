...
export default function MainPage({ currentLocation, setCurrentLocation }) {
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(async () => {
      ...
      try {
        const response = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:8000'}/roam`, {
          method: "POST",
  ...
  const handleAIRequest = async (inputText) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:8000'}/generate-route`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
